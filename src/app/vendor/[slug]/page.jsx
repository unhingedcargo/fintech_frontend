...
export default function CustomerDetails(params) {
  ...
  const updateData = async () => {
    setLoader(true);
    const UPDATE_VENDOR_URI = `${BASE_URL}/contact/update/${slug}`
    ...
    .then(result => {
      setAlert({show:true, type:"success", message:`${vendor[0].display_name} Updated Successfully..!!`});
      console.log(vendor);
      setTimeout(() => {
        setAlert({show:false, type:"success", message:""});
      }, 3000);
    ...
    .catch(error => {
      console.log(error);
      router.push(`/vendor?message=Something Went Wrong!`); 
    })
    .finally (() => {
      setLoader(false);
    });
  ...
  const deleteData = async () => {
    setLoader(true);
    // const DELETE_VENDOR_URI = `https://fintech-backend-08wx.onrender.com/api/contact/delete/${slug}`
    const DELETE_VENDOR_URI = `${BASE_URL}/contact/delete/${slug}`
    const deleteName = vendor[0].display_name;
    try{
      const res = await fetch(DELETE_VENDOR_URI, {
        method:"DELETE",
        headers:{
          "content-type" : "application/json; charset=UTF-8",
        },
      ...
      router.push(`/vendor?message=${encodeURIComponent(deleteName)} Deleted Successfully`); 
    } catch(error){
      ...
      <div className='flex'>
            ...
            <div className="card card-border bg-base-100 w-[80%]" key={index}>
              <div className="card-body">
                ...
                <button className="btn btn-soft" onClick={() => router.push("/vendor")}>Back</button>
                </div>
              </div>
            </div>
					  )}
